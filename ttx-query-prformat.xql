...
Or search...<p/>
<form method="get" enctype="multipart/form-data" action="ttx-query-prformat.xql">
<table>
...
return
<tr>
<td><a href="ttx-detail.xql?iBugNumber={$bug/defect-number}">{$bug/defect-number}</a></td>
<td>{$bug/summary}</td>
<td>{$bug/priority}</td>
<td>{$bug/component}</td>
<td>{$bug/reference}</td>
<td>--</td>
<td>--</td>
<td>{$bug/defect-status}</td>
<td>{$bug/disposition}</td>
<td>{$bug/entered-by}</td>
<td>{$bug/date-entered}</td>
<td>{$bug/reported-by-record/version-found}</td>
</tr>
}
</table></div>
else ()
